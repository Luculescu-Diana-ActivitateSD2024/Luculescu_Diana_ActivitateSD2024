...
Galerie initGalerie(int cod, const char* nume, float pret) {
	Galerie g;
	g.cod = cod;
	g.nume = (char*)malloc(strlen(nume) + 1);//sizeof(char)=1;
	strcpy(g.nume, nume);
	g.pretIntrare = pret;
	return g;
}
...
void inserareGalerieInTabela(HashTable hash, Galerie galerie) {
	if (hash.dim > 0)
	{
		...
		if (hash.vector[pozitie] != NULL) //acest if-else este aici doar pt a prezenta situatia de conflict
		{
...
//cautare galerie dupa cheie(acum e compusa)
Galerie cautaGalerieDupaCheie(int cod,const char* nume, HashTable tabela) {
	//ar trb verificat ca avem tabela
